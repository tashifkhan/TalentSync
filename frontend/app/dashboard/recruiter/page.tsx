...
export default function RecruiterDashboard() {
  ...
  const [selectedResume, setSelectedResume] = useState<RecruiterResume | null>(
    null
  );
  ...
  const {
    ...
    error,
    refetch,
  } = useRecruiterResumes(searchQuery);
  ...
  // Debounced search could be implemented here, but for now relying on query key change
  // React Query handles deduping, but frequent key changes trigger fetches.
  // Ideally we should debounce setSearchQuery update.
                        ...
                        <div className="backdrop-blur-lg bg-white/5 rounded-lg p-6 border border-white/10">
                          ...
                          <div className="space-y-6">
                            {selectedResume.analysis.workExperience
                              .filter(
                                (work: any) =>
                                  work.role || work.company_and_duration
                              )
