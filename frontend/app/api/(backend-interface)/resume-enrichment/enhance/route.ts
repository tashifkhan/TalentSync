...
export async function POST(request: NextRequest) {
  try {
    ...
    const backendResponse = await fetch(
      `${backendUrl}/api/v1/resume/enrichment/enhance`,
      {
        method: "POST",
        headers: {
    ...
    if (!backendResponse.ok) {
      ...
      console.error("Enrichment enhance API - Backend error:", {
        status: backendResponse.status,
        error: errorText.substring(0, 500),
      });

      let errorMessage = "Failed to generate enhanced descriptions";
      try {
    ...
    console.error("Enrichment enhance API - Error:", error);
