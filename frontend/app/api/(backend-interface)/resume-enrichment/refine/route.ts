...
import type { EnhancementPreview, RefinementInput } from "@/types/enrichment";
...
export async function POST(request: NextRequest) {
  try {
    ...
    const { resumeId, refinements } = body;
    ...
    if (!refinements || !Array.isArray(refinements) || refinements.length === 0) {
      return NextResponse.json(
        { success: false, message: "At least one refinement is required" },
        { status: 400 }
      );
    }
    ...
    const backendResponse = await fetch(
      `${backendUrl}/api/v1/resume/enrichment/refine`,
      {
        method: "POST",
        headers: {
    ...
    if (!backendResponse.ok) {
      ...
      console.error("Enrichment refine API - Backend error:", {
        status: backendResponse.status,
        error: errorText.substring(0, 500),
      });

      let errorMessage = "Failed to refine enhancements";
      try {
    ...
    const responseData: EnhancementPreview = await backendResponse.json();
    return NextResponse.json({ success: true, data: responseData });
  } catch (error) {
    console.error("Enrichment refine API - Error:", error);
