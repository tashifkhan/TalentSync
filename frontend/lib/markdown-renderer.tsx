      ...
      result.push(
        <ul key={`ul-${key++}`} className="list-disc ml-4 space-y-1 mb-2">
          {currentList.map((item, i) => (
            <li key={i} className="text-brand-light/80">
  ...
  lines.forEach((line) => {
    ...
    if (trimmed.startsWith("* ") || trimmed.startsWith("- ")) {
      ...
      result.push(
        <p
          key={`heading-${key++}`}
          className="font-bold text-brand-primary mb-2"
        >
      ...
      result.push(
        <p
          key={`h3-${key++}`}
          className="font-semibold text-brand-light mb-2 text-sm"
        >
      ...
      result.push(
        <p
          key={`h2-${key++}`}
          className="font-bold text-brand-light mb-3 text-base"
        >
          {parseMarkdownText(trimmed.replace(/^#+\s*/, ""))}
        </p>,
      );
      ...
      result.push(
        <p
          key={`p-${key++}`}
          className="mb-2 text-brand-light/80 leading-relaxed"
        >
